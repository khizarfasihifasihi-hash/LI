...
with st.sidebar:
    ...
    image_engine = st.radio(
        ...
        ["StabilityAI (Stable Diffusion 3.5)", "Mistral (Agents API + Flux Ultra)"],
        index=0,
        help="StabilityAI offers high-quality Stable Diffusion models. The Mistral option "
    ...
    if image_engine.startswith("StabilityAI"):
        stability_ai_api_key = st.text_input(
            "StabilityAI API Key",
            value=os.environ.get("STABILITY_API_KEY", ""),
            type="password",
            help="Get a key at https://platform.stability.ai/account/api-keys. "
                 "Required when using StabilityAI image engine.",
        )
    ...
    st.caption(
        "🖼️ StabilityAI requires an API key. Mistral's image tool needs a "
...
def generate_image_bytes_stability_ai(prompt: str, width: int, height: int, api_key: str) -> bytes:
    """Generate an image from a text prompt using StabilityAI's API.
    Returns raw image bytes (or raises on failure/timeout, which the caller should catch)."""
    if not api_key:
        raise ValueError("StabilityAI API key is required for this image engine.")
...
def generate_image(prompt: str, width: int, height: int, engine: str, stability_ai_key: str, mistral_key: str, mistral_model: str) -> bytes:
    ...
    return generate_image_bytes_stability_ai(prompt, width, height, stability_ai_key)


def render_image_block(prompt: str, width: int, height: int, state_key: str, engine: str, stability_ai_key: str, mistral_key: str, mistral_model: str) -> None:
        ...
        with st.spinner("Generating a new image..."):
            try:
                st.session_state[state_key] = generate_image(
                    prompt, width, height, engine, stability_ai_key, mistral_key, mistral_model
                )
                st.rerun()
...
def _make_image_for_entry(history_entry: dict, website: str, topic: str, category: str, tone: str, post_text: str,
                           image_engine: str, stability_ai_key: str, mistral_api_key: str, mistral_image_model: str) -> None:
    ...
    with st.spinner(f"Generating a matching image with {image_engine.split(' ')[0]}..."):
        try:
            st.session_state[state_key] = generate_image(
                ...
                image_engine, stability_ai_key, mistral_api_key, mistral_image_model,
            )
...
def generate_video_bytes(scene_prompts: list, width: int, height: int, image_engine: str,
                          stability_ai_key: str, mistral_key: str, mistral_model: str, seconds_per_scene: int = 3,
    ...
    for i, scene_prompt in enumerate(scene_prompts):
        ...
        img_bytes = generate_image(scene_prompt, width, height, image_engine, stability_ai_key, mistral_key, mistral_model)
...
def _make_video_for_entry(history_entry: dict, website: str, topic: str, category: str, tone: str, post_text: str,
                           image_engine: str, stability_ai_key: str, mistral_api_key: str, mistral_image_model: str,
    ...
    try:
        video_bytes = generate_video_bytes(
            ...
            stability_ai_key, mistral_api_key, mistral_image_model, seconds_per_scene=seconds_per_scene,
...
if generate:
    ...
    elif (generate_image_toggle or generate_video_toggle) and image_engine.startswith("StabilityAI") and not stability_ai_api_key:
        st.error("Please enter your StabilityAI API key in the sidebar, or switch to the Mistral image engine.")
    elif (generate_image_toggle or generate_video_toggle) and image_engine.startswith("Mistral") and not mistral_api_key:
        st.error("Please enter your Mistral API key in the sidebar, or switch to the StabilityAI image engine.")
    else:
        emoji_instruction = (
    ...
    if use_clicked and chosen_text:
        ...
        if cfg["generate_image_toggle"]:
            _make_image_for_entry(
                ...
                chosen_text, cfg["image_engine"], cfg["stability_ai_api_key"], cfg["mistral_api_key"], cfg["mistral_image_model"],
            )
